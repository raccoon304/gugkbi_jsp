...
<body>
	<div id="container">
		<!-- method는 기본적으로 get임. post를 쓰기위해서는 지정해줘야함. -->
	
<%-- 	<form action="<%= request.getContextPath()%>/02_postMethod.do method="post"> --%>
		<!-- 또는 -->			
		<%-- <form action="<%=ctxPath%>/02_postMethod.do method="post">	 --%>
		<!-- 또는 -->
		<%-- <form action="${pageContext.request.contextPath}/02_postMethod.do method="post"> --%>
		<!-- 또는 -->
		<form action="${pageContext.request.contextPath}/02_postMethod.do" method="post">
...
<%-- !! 중요 !!
        ...
        현재 <%= ctxPath%> 이  /JSPServletBegin 이므로 <%= ctxPath%>/02_postMethod.do 의 뜻은 http://localhost:9090/JSPServletBegin/01_getMethod.do 이라는 말이다.        
		form 태그에서 submit(전송)을 하면 http://localhost:9090/JSPServletBegin/02_postMethod.do 으로 데이터를 전송시킨다.
        ...
        /JSPServletBegin/01_getMethod.do 은 누가 처리를 해주는지는 배치서술자인 web.xml 에 기술되어져 있어야 한다.
       	web.xml 에 가보면 <servlet-mapping>에 URL 패턴으로 /02_postMethod.do 가 기술되어져 있고, 이어서 실제로 처리를 해주는 <servlet-class>에 클래스명이 기술되어져 있다.
