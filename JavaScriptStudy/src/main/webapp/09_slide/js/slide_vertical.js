window.onload = () => {
    ...
    const image_height = 300;//한번 이동시 해당 숫자만큼 이동.
    ...
    const func_next = () => {
        ...
        if(current_indexno < max_indexno) { //이미지의 현재 인덱스 번호가 마지막(지금은 3)이 아닌경우. 
            ...
            positionValue -= image_height; // images 의 위치를 왼쪽으로 이동시키도록 image_height를 빼줘야함.
         
            div_images.style.transform = `translateY(${positionValue}px)`;// x축 방향(가로)
    ...
    const func_previous = () => {
        ...
        if(current_indexno > 0) {  //이미지의 현재 인덱스 번호가 처음(지금은 3)이 아닌경우. 
            ...
            positionValue += image_height; // images 의 위치를 오른쪽으로 이동시키도록 image_height를 더해줘야함.
