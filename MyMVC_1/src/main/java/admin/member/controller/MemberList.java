	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(loginUser != null && "admin".equals(loginUser.getUserid()) ) {
			...
			if( searchType == null ||
			   ...
			   (!searchType.equals("userid")) && 
			   (!searchType.equals("email")) ) {
				searchType = "";
			}// GET방식에서 검색을 직접적으로 null을 넣을 수 있어 이렇게 빼는게 좋음
			...
			if( sizePerPage == null ||
			   (!sizePerPage.equals("10")) && 
			   (!sizePerPage.equals("5")) && 
			   (!sizePerPage.equals("3")) ) {
				sizePerPage = "10";
			}
			...
			System.out.println("확인용 서치타입 "+ searchType);
			System.out.println("확인용 서치워드 "+ searchWord);
			System.out.println("확인용 사이즈퍼페이지 "+ sizePerPage);
			System.out.println("확인용 현재페이지 "+ currentShowPageNo);
	        ...
	        1개 페이지당 3개행 또는 5개행 또는 10개행을 보여주는데 만약에 1개 페이지당 5개행을 보여준다라면 총 몇개 블럭이 나와야 할까? 
	       ...
	       */
			...
			while(!(loop > blockSize || pageNo > totalPage )) {
				...
				pageNo++;	// 1  2  3  4  5  6  7 ...
							// 11 12 13 14 15 16 17 ...
							// 21 22 23 24 25 26 27 ...
							// 31 32 33 34 35 36 37 ...
					//	(잘못됨) 41 42 43 44 45 46 47 ... 		<- 가정상 42에서 끝나야되는데, 멈추지않음. 그래서 pageNo > totalPage 넣어서 멈추게 하면 아래처럼 나옴
					//	(정상)  41 42
			}// EoP while(!(loop > blockSize || pageNo > totalPage )){}
			...
			super.setViewPage("/WEB-INF/admin/member/memberList.jsp");	//따로 명시를 하지 않았음으로, GET 방식으로 접근
		} else {
