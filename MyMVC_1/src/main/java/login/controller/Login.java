...
public class Login extends AbstractController {
   ...
   public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
      ...
      //###### 확인용 saveid : on	    <= 체크박스에 체크되어진 상태 
      //###### 확인용 saveid : null	<= 체크박스에 체크되지않은 상태 
      ...
      if(loginUser != null) {
         ...
         // ~~~~~~~~~~ "아이디저장" 을 쿠기(Cookie)를 사용하여 처리하기 위한 것 시작 ~~~~~~~~~~ //
         if(saveid != null) {//아이디저장 체크박스에 체크가 되어진 상태
        	 Cookie cookie = new Cookie("saveid", userid);
        	 				//new Cookie("쿠키명", 쿠키값);
        	// Cookie 클래스 임포트시 jakarta.servlet.http.Cookie 임.
        	cookie.setMaxAge(24*60*60); // 쿠키의 수명1은 1일로 설정 
        	cookie.setPath("/");// 쿠키가 브라우저에서 전송될 URL 경로 범위(Path)를 지정하는 설정임     	
/* 
  			Path를 /로 설정하면:
            /, /login, /user/profile, /admin 등 모든 서브경로에 대해 이 쿠키가 브라우저에서 자동으로 전송된다.
          
            Path를 /login으로 설정하면: /login, /login/check, /login/form 등 /login으로 시작하는 경로에서만 쿠키가 전송된다.
          						   /, /main, /user 등의 다른 경로에서는 이 쿠키는 사용되지 않음.   
*/
        	response.addCookie(cookie);//접속한 클라이언트 PC의 웹 브라우저로 쿠키를 보내줌 
         
			
			
			 Cookie cookie2 = new Cookie("test", "TEST"); cookie2.setMaxAge(12*60*60);
			 cookie2.setPath("/"); response.addCookie(cookie2);
			 
			 Cookie cookie3 = new Cookie("example", "EXAMPLE");
			 cookie3.setMaxAge(12*60*60); cookie3.setPath("/");
			 response.addCookie(cookie3);
			 
         }
         else {	// 아이디저장 체크박스에 체크가 해제 되어진 상태
			 ...
			 Cookie cookie2 = new Cookie("test", null); // 값은 null 또는 빈 문자열
			 cookie2.setMaxAge(0); //0초 => 즉시 만료 cookie.setPath("/");// 쿠키가 브라우저에서 전송될 URL경로 범위(Path)를 지정하는 설정임 
        	 cookie2.setPath("/");// 쿠키가 브라우저에서 전송될 URL 경로 범위(Path)를 지정하는 설정임  
			 response.addCookie(cookie2);//접속한 클라이언트 PC의 웹 브라우저로 쿠키를보내줌
			 
			 Cookie cookie3 = new Cookie("example", null); // 값은 null 또는 빈 문자열
			 cookie3.setMaxAge(0); //0초 => 즉시 만료 cookie.setPath("/");// 쿠키가 브라우저에서 전송될 URL경로 범위(Path)를 지정하는 설정임 
        	 cookie3.setPath("/");// 쿠키가 브라우저에서 전송될 URL 경로 범위(Path)를 지정하는 설정임   
			 response.addCookie(cookie3);//접속한 클라이언트 PC의 웹 브라우저로 쿠키를 보내줌
