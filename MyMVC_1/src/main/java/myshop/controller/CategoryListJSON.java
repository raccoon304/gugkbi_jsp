...
public class CategoryListJSON extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    if(categoryList.size() > 0) {
	    	...
	    	for(CategoryDTO cdto : categoryList) {
	            
	        JSONObject jsonObj = new JSONObject(); // {}
	            
	        	jsonObj.put("cnum", cdto.getCnum());   // {"cnum":1}
	            jsonObj.put("code", cdto.getCode());   // {"cnum":1, "code":"100000"} 
	            jsonObj.put("cname", cdto.getCname()); // {"cnum":1, "code":"100000", "cname":"전자제품"}  
