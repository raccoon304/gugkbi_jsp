...
<script type="text/javascript">
	...
	let total_fileSize = 0; // 펌부한 파일의 총량을 누적하는 용도 
	...
	$(function(){
		...
		}); // EoP $('input#spinnerPqty').spinner({})
		
		// ==> 제품 이미지 파일선택을 선택하면 화면에 이미지를 보여주기 시작 <<== //
		$('input.img_file').bind('change',function(e){
			//alert("이미지 파일명이 바뀌었습니다.");
           ...
           */
           ...
           fileReader.onload = function(){
	          ...
	          */
	          //jQuery를 순수한 JS로 위에서 변환했기에 반드시 JS로 해줘야함
		...
		<%-- ==> 제품 이미지 파일선택을 선택하면 화면에 이미지를 보여주기 끝 <<== --%>

		// == 제품설명서 파일첨부(선택)을 선택하면 total_fileSize 에 파일크기 누적하기 시작 == <<
		$(document).on("change","input:file[name='prdmanualFile']", function(e){
			const input_file = $(e.target).get(0);
			total_fileSize += input_file.files[0].size;
		});
		...
		<%-- === jQuery를 사용하여 드래그앤드롭을 통한 파일 업로드 시작 --%>
		let file_arr_nujuk = [];// 첨부되어진 추가이미지 파일 정보를 누적하여 저장하는 용도
		
		// == 파일 Drag & Drop 만들기 == // 
		$('div#fileDrop').bind('dragenter', function(e){/* "dragenter" 이벤트는 드롭대상인 박스 안에 Drag 한 파일이 최초로 들어왔을 때 */ 
			e.preventDefault();
			<%-- 
            ...
            --%>
		}).bind('dragover', function(e){/* "dragover" 이벤트는 드롭대상인 박스 안에 Drag 한 파일이 머물러 있는 중일 때. 필수이벤트이다. dragover 이벤트를 적용하지 않으면 drop 이벤트가 작동하지 않음 */
        	e.preventDefault();
        	e.stopPropagation();
        	$(this).css("background-color","#ffd8d8");
        }).bind('dragleave', function(e){/* "dragleave" 이벤트는 Drag 한 파일이 드롭대상인 박스 밖으로 벗어났을 때  */
        	e.preventDefault();
        	e.stopPropagation();
        	$(this).css("background-color","#fff");
        }).bind('drop', function(e){/* "drop" 이벤트는 드롭대상인 박스 안에서 Drag 한것을 Drop(Drag 한 파일(객체)을 놓는것) 했을 때. 필수이벤트이다. */	
        	e.preventDefault();
            ...
            */
            ...
            if(files != null && files != undefined){
	            ...
	            for(let i=0; i<files.length; i++){
                	...
                	*/
	            	...
	            	if( !(f.type == 'image/jpeg' || f.type == 'image/png') ) {
                        ...
                        alert("10MB 이상인 이미지 파일은 업로드 할 수 없습니다.");
                        $(this).css("background-color", "#fff");
                        return; // 종료
                     }
                     else {
	            ...
	            }// EoP for(let i=0; i<files.length; i++){}
...
//                  console.log(fileSize);
                	...
                	*/
                	...
                	// ``(벡틱) 사용시 확장자가 js라면 괜찮지만, jsp 속에 있는 js는 $해버리면 requestScope처럼 인식을 해버림. 
                	// 즉, ${fileName} 이런식으로 써버리면 ${requestScope.fileName}으로 읽어버려서 안댐 
                	// 그래서 \${fileName} 앞에 역슬래시를 써서 표시해줘야함.
                    ...
                    $(this).append(v_html);
                 	// ===> 추가이미지 파일선택을 선택하면 화면에 이미지를 미리 보여주기 시작 <=== //
                    
                    // console.log(imgfile);
                    // File {name: 'berkelekle덩크04.jpg', lastModified: 1605506138000, lastModifiedDate: Mon Nov 16 2020 14:55:38 GMT+0900 (GMT+09:00), webkitRelativePath: '', size: 41931, …}   
                    // File {name: 'berkelekle심플V넥02.jpg', lastModified: 1605506138000, lastModifiedDate: Mon Nov 16 2020 14:55:38 GMT+0900 (GMT+09:00), webkitRelativePath: '', size: 58889, …} 
	            ...
	            }// EoP for(let imgfile of file_arr){}
            }// EoP if(files != null && files != undefined){}
        ...
        });// EoP }).bind('drop', function(e){}
		
        
        
		// == Drop 되어진 파일 목록 제거하기 시작 == //
		
		
		/* 
		// 아래처럼 하면 안잡힘. 
		$('span.delete').bind('click', function(e){
			alert("삭제를 클릭하셨습니다.");
		});
		 */
		 //그래서 이처럼 잡아줘야 댐. (순수 JS 문법 )
		$(document).on("click", "span.delete", function(e){
			//alert("삭제를 클릭하셨습니다.");
			
			// 위의 여러 span tag 중 진짜 선택한 span tag 
		...
		// == Drop 되어진 파일 목록 제거하기 끝 == //
		
		
		
		<%-- === jQuery를 사용하여 드래그앤드롭을 통한 파일 업로드 끝 --%>
		// ========= 제품등록하기 시작 ========= //
		$(document).on("click", "input:button[id='btnRegister']", function(){
			...
			// 바닐라 JS로 짜면. 
			//document.querySelectorAll(".infoData").forEach(function(elmt, index, array){ });
			$('.infoData').each(function(index, elmt){ 
				...
				if(val == ""){
					...
					return false;
					// forEach 는 break(중단) 기능이 없으나, 
					// each 는 break(중단) 기능이 있는데 바로 return false; 이다.
					
				}// EoP if(val == ""){}
			}); // EoP $('.infoData').each(function(index, elmt){ 
			...
			if(is_infoData_OK){
                ...
                const formData = new FormData($("form#myform").get(0));  // 폼에 작성된 모든것       
                또는
                const formData = new FormData($("form#myform")[0]);  // 폼에 작성된 모든것
                // jQuery선택자.get(0) 은 jQuery 선택자인 jQuery Object 를 DOM(Document Object Model) element 로 바꿔주는 것이다. 
               	// DOM element 로 바꿔주어야 순수한 javascript 문법과 명령어를 사용할 수 있게 된다. 
         
               	또는
                const formData = new FormData(document.getElementById('myform'));  // 폼에 작성된 모든것
                ...
                formData.append("imgFile", $("input#imgFile")[0].files[0]);
              	...
              	*/
                const formData = new FormData($('form[name="prodInputFrm"]').get(0));  
				// $('form[name="prodInputFrm"]').get(0) 폼에 작성된 모든 데이터를 보내기. 
				
				//추가 이미지파일을 추가했을 경우
                if(file_arr_nujuk.length > 0 ){
                	
                	// ====== formData 속에 추가이미지의 파일 개수 넣어주기. 
                	formData.append("attachCount",file_arr_nujuk.length);
                	
                	for(let i=0; i<file_arr_nujuk.length; i++){         
                		// 추가 이미지 파일의 용량크기를 누적하기. 
                		total_fileSize += file_arr_nujuk[i].size;
                		
                		
                		// ==== formData 속에 추가 이미지 파일 넣어주기 ====
                		// formData.append("key", value값);  "key" 가 name 값이 되어진다.
                		formData.append("attach"+i, file_arr_nujuk.length[i]);
                	}// EoP for(let i=0; i<file_arr_nujuk.length; i++){}
                	
                	// ===== 첨부한 파일의 누적된 총량의 크기를 확인하여 20MB 이상이면 전송불가 하도록 한다. ===== // 
                	if(total_fileSize >= 20*1024*1024){
                		alert("첨부한 파일 총합의 크기가 20MB이상인 경우 제품등록을 할 수 없습니다.");
                		return;
                	}
                	//------------------------------------------------------------------------------------------------------------------//
                	// === formData를 전송(submit)해야 하므로 ajax를 사용해야 함. 
                	
                	$.ajax({
                		<%-- url : "<%= ctxPath%>/shop/admin/productRegister.up", --%>
                        url: "${pageContext.request.contextPath}/admin/shop/productRegister.up",
                        type: "post", //--> form태그 내 method를 post방식으로 보내기로 이미함. 
                        data: formData,
                ...
                }// EoP if(file_arr_nujuk.length > 0 ){}

			}// EoP if(is_infoData_OK){}
		...
		// === 취소하기 === //
       	$("input[type='reset']").click(function(){
          	$("span.error").hide();
          	file_arr_nujuk = [];  // 첨부되어진 파일 정보를 누적하여 담아둘 배열을 초기화 하기
          	$("div#fileDrop").empty();
          	$("img#previewImg").hide();
       	});// end of $('input[type="reset"]').click()----------------
...
<div align="center" style="margin-bottom: 20px;">
   ...
   <form name="prodInputFrm" method="post" enctype="multipart/form-data"> 
      ...
      <tbody>
         <tr>
            ...
            <td width="75%" align="left" style="padding-top: 10px;" >
               <select name="fk_cnum" class="infoData">
                  ...
                  <!-- 카테고리를 동적으로 보여줘야하니까 방법은 2개임 ajax로 보여주거나, controller단에서 DB 정보를 읽어다가 view단에 뿌려주면댐. -->
                  <c:if test="${not empty requestScope.categoryList }">
                     <c:forEach var="categoryDto" items="${requestScope.categoryList }">
                        <option value="${categoryDto.cnum }">${categoryDto.cname }</option>
                     </c:forEach>
                  </c:if>
         ...
         <tr>
            ...
            <td width="75%" align="left" style="border-top: hidden; border-bottom: hidden;">
               <select name="fk_snum" class="infoData">
                  ...
                  --%>
                  <!-- 제품스펙를 동적으로 보여줘야하니까 방법은 2개임 ajax로 보여주거나, controller단에서 DB 정보를 읽어다가 view단에 뿌려주면댐. -->
                  <c:if test="${not empty requestScope.specList }">
                     <c:forEach var="specDto" items="${requestScope.specList }">
                        <option value="${specDto.snum }">${specDto.sname }</option>
                     </c:forEach>
                  </c:if>
         ...
         <%-- ==== 추가이미지파일을 마우스 드래그앤드롭(DragAndDrop)으로 추가하기 ==== --%>
          <tr>
                ...
                <td>
                   <span style="font-size: 10pt;">파일을 1개씩 또는 여러개씩 마우스로 끌어 오세요</span>
