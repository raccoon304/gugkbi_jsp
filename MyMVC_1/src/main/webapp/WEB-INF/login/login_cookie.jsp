...
<%-- 
<% 
/* 쿠키(Cookie)값을 읽어올때 자바를 이용하는 경우 시작 */
	String saveid = "";
	Cookie[] arrCookie = request.getCookies(); // 쿠키 읽어오기 
	if(arrCookie != null){
		for(Cookie cookie : arrCookie){
			//System.out.println("쿠키 : " + cookie.getName() + ", 쿠키값 : "+ cookie.getValue());
			/*
			쿠키 : JSESSIONID, 쿠키값 : 3A9D1FC52396F9E2639CF394DC62431F
			쿠키 : test, 쿠키값 : TEST
			쿠키 : example, 쿠키값 : EXAMPLE
			쿠키 : saveid, 쿠키값 : sonyd
			*/
...
/* 쿠키(Cookie)값을 읽어올때 자바를 이용하는 경우 끝 */
%>
 --%>              
               <tr>
                  <td>ID</td>
                  <%-- 쿠키(Cookie)값을 읽어올때 자바를 이용하는 경우 --%>                  
                  <%-- <td><input type="text" name="userid" id="loginUserid" size="20" autocomplete="off" value="<%= saveid%>"/></td> --%>

                  <%-- 또는쿠키(Cookie)값을 읽어올때 자바스크립트를 이용하는 경우 --%>
