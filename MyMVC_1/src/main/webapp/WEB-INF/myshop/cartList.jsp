...
<script type="text/javascript">
  ...
  const arr_oqty = []; // 현재주문수량 값을 기억하는 용도 
  ...
  $(function(){
		...
		// === 현재 주문수량에 키보드로 값을 입력할 경우 숫자가 아니든지 또는 0 ~ 100 까지 범위의 값이 아니라면 === //
		$('input.oqty').on("keyup", function(e){
			...
			if(!bool) {
				alert("수정하시려는 수량은 0개 이상 100개 이하 이어야 합니다.");
				$(e.target).val("1");
			}
			else {
				if($(e.target).val() > 100) {
		...
		});// end of $('input.oqty').on("keyup", function(e){}----------------
  ...
  function goOqtyEdit(btn){
	  ...
	  const idx = $('button.updateBtn').index(btn); // 인덱스
  //  alert(idx);
	  
	  const cartno = $('input:text[class="cartno"]').eq(idx).val(); // 장바구니번호
  //  alert(cartno);
	  ...
	  if(checkCtn < 1) {
		  ...
		  for(let i=0; i<allCnt; i++){
			  if( $('input:checkbox[name="pnum"]').eq(i).prop("checked") ) {
				  ...
				  console.log("주문량 : ", $('input[name="oqty"]').eq(i).val());
				  ...
				  arr_oqty.push($('input[name="oqty"]').eq(i).val());
		...
		<%--  
		  for(let i=0; i<checkCtn; i++){
			   console.log("확인용 제품번호: " + arr_pnum[i] + ", 주문량: " + arr_oqty[i] + ", 잔고량: " + arr_pqty[i] + ", 장바구니번호 : " + arr_cartno[i] + ", 주문금액: " + arr_totalPrice[i] + ", 포인트: " + arr_totalPoint[i]); 
			 /*  
			    확인용 제품번호: 5, 주문량: 2, 잔고량: 100, 장바구니번호 : 5, 주문금액: 66000, 포인트: 40
                확인용 제품번호: 3, 주문량: 1, 잔고량: 20, 장바구니번호 : 4, 주문금액: 10000, 포인트: 5
                확인용 제품번호: 36, 주문량: 12, 잔고량: 100, 장바구니번호 : 2, 주문금액: 12000000, 포인트: 720 
			 */
		  }// end of for------------------
		--%>
		  ...
		  for(let i=0; i<checkCtn; i++){
			  ...
			  if( Number(arr_pqty[i]) < Number(arr_oqty[i]) ) {
			      ...
			      alert("제품번호 " + arr_pnum[i] + " 의 주문개수가 잔고개수 보다 더 커서 진행할 수 없습니다.");
			  	  location.href="javascript:history.go(0)";  // 새로고침
			  	  return;  // goOrder() 함수 종료
			  }
		  }// end of for------------------
		  
		  let n_sum_totalPrice = 0; // 주문총액
		  ...
		  if(current_coin < n_sum_totalPrice) {
			  ...
			  if( confirm("총주문액 "+ n_sum_totalPrice.toLocaleString('en') + "원을 주문하시겠습니까?") ) {
				  ...
				  $.ajax({
					  ...
					  data:{"n_sum_totalPrice":n_sum_totalPrice, // 주문총액
						    "n_sum_totalPoint":n_sum_totalPoint, // 주문총포인트
						    "str_pnum_join":arr_pnum.join(),     // 제품번호를 다 합친것 
						    "str_oqty_join":arr_oqty.join(),     // 주문량을 다 합친것
						    "str_totalPrice_join":arr_totalPrice.join(), // 주문가격을 다 합친것
						    "str_cartno_join":arr_cartno.join()  // 장바구니번호를 다 합친것 
					       },
		   ...
		   <tbody>
			   	  ...
			   	  <c:forEach var="cartDto" items="${requestScope.cartList}" varStatus="status">
			   	     <tr>
			   	         ...
			   	         <td align="center"> <%-- 제품이미지1 및 제품명 --%>
			   	            <a href="${pageContext.request.contextPath}/shop/prodView.up?pnum=${cartDto.fk_pnum}">
			   	               <img src="${pageContext.request.contextPath}/images/${cartDto.pdto.pimage1}" class="img-thumbnail" width="130px" height="100px" />
			   	            </a>
			   	            <br>
			   ...
			   <tr>
			         ...
			         <button type="button" class="btn btn-outline-dark btn-sm mr-3" onclick="goOrder()">주문하기</button>
			   	     <a class="btn btn-outline-dark btn-sm" href="${pageContext.request.contextPath}/shop/mallHomeMore.up" role="button">계속쇼핑</a> 
			   	  </td>
			   </tr>
