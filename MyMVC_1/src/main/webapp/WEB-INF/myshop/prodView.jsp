...
<script type="text/javascript">
	$(function(){
        ...
        $('div.carousel div.carousel-item').each(function(index, elmt){
           ...
           --%>
            ...
            <%-- 즉, 다음번 클래스가 carousel-item 인 div 의 자식태그인 img 태그를 복사한 img 태그를 만들어서 $(elmt) 태그속에 있는 기존 img 태그 뒤에 붙여준다. --%>
	...
	}// EoP function goOrder(){}
	
	function modal_content(img){
		//alert("모달창 속에 이미지를 넣어주어야 함.");
		//alert(img.src);
	...
	}// EoP function modal_content(this){}
	
	// ==== 주문량에 대한 유효성 검사 ==== //
	// *** 장바구니 담기 (Ajax 를 사용하지 않고 form 태그를 사용하여 처리해 보겠습니다.) ***// 
  	function goCart(){
  		...
  		if(!bool){ // 숫자 이외의 값이 들어오거나 첫번째 숫자가 0 인 경우
  		...
  		if(Number(oqty) > Number(${requestScope.pdto.pqty})){
  			// 주문개수가 잔고개수 보다 클 경우
...
<div style="width: 95%;">
   	...
   	<div class="row my-3 text-left">
      	<div class="col-md-6">
          	<div class="flip-card">
              	...
              	<div class="flip-card-front">
              		<!-- 전면 -->
                  	<img src="${pageContext.request.contextPath}/images/${requestScope.pdto.pimage1}" class="img-fluid" style="width:100%;" />
               	</div>
               	<div class="flip-card-back">
               		<!-- 후면 -->
                   	<img src="${pageContext.request.contextPath}/images/${requestScope.pdto.pimage2}" class="img-fluid" style="width: 100%;" />
               	</div>
            	</div>
         	</div>
      	</div>
      	...
      	<div class="col-md-6 pl-5">
          	<ul class="list-unstyled">
            	<li>
            		<span style="color: red; font-size: 12pt; font-weight: bold;">
            			${requestScope.pdto.spdto.sname}
            		</span>
            	</li>
	            <li>제품번호: ${requestScope.pdto.pnum}</li>
	            <li>제품이름: ${requestScope.pdto.pname}</li>
	            <li>제조회사: ${requestScope.pdto.pcompany}</li>
	            <li>제품설명서: 
              		<c:if test="${requestScope.pdto.prdmanual_orginFileName ne '없음'}">
                 		<a href="${pageContext.request.contextPath}/shop/fileDownload.up?pnum=${requestScope.pdto.pnum}">
                 			${requestScope.pdto.prdmanual_orginFileName}
                 		</a>
	              	</c:if>
	             	<c:if test="${requestScope.pdto.prdmanual_orginFileName eq '없음'}">
	                 	첨부파일없음
	              	</c:if>
            	</li>
              	<li>제품정가: <span style="text-decoration: line-through;"><fmt:formatNumber value="${requestScope.pdto.price}" pattern="###,###" />원</span></li>
              	<li>제품판매가: <span style="color: blue; font-weight: bold;"><fmt:formatNumber value="${requestScope.pdto.saleprice}" pattern="###,###" />원</span></li>
	            <li>포인트: <span style="color: green; font-weight: bold;">${requestScope.pdto.point} POINT</span></li>
	            <li>잔고갯수: <span style="color: maroon; font-weight: bold;">${requestScope.pdto.pqty} 개</span></li>
          	...
          	<form name="cartOrderFrm">       
	            ...
	            <input type="hidden" name="pnum" value="${requestScope.pdto.pnum}" />
         	</form>   
      	</div>
   	</div>
    ...
    <div style="display: flex; position: absolute; top: 30%; left: 37%; border: solid 0px blue;">
   	...
   	<%-- === 추가이미지 보여주기 시작 === --%>
   	<c:if test="${not empty requestScope.pdto.imgList}">        
   	
	    <%-- /////// 추가이미지 캐러젤로 보여주는 것 시작 //////// --%>
	    <div class="row mx-auto my-auto" style="width: 100%;">
	    	<div id="recipeCarousel" class="carousel slide w-100" data-ride="carousel">
	        	<div class="carousel-inner w-100" role="listbox">
	            	<c:forEach var="imgfilename" items="${requestScope.pdto.imgList}" varStatus="status">
	                	<c:if test="${status.index == 0}">
	                    	<div class="carousel-item active">
	                        	<img class="d-block col-3 img-fluid" src="${pageContext.request.contextPath}/images/${imgfilename}" style="cursor: pointer;" data-toggle="modal" data-target="#add_image_modal_view" data-dismiss="modal" onclick="modal_content(this)" />     
	                        </div>
	                    </c:if>
	             		<c:if test="${status.index > 0}">
   ...
   <div class="jumbotron mt-5">
        ...
        <div class="text-left" style="margin-top: -5%;">
           <p class="h4 bg-secondary text-white w-50">${requestScope.pdto.pname} 제품의 특징</p>
           <p>${requestScope.pdto.pcontent}</p>   
       </div>
    
      <div class="row">
         <div class="col" style="display: flex">
               <h3 style="margin: auto">
                  <i class="fas fa-thumbs-up fa-2x" style="cursor: pointer;" onclick="golikeAdd('${requestScope.pdto.pnum}')"></i> 
         ...
         <div class="col" style="display: flex">
             <h3 style="margin: auto">
                <i class="fas fa-thumbs-down fa-2x" style="cursor: pointer;" onclick="godisLikeAdd('${requestScope.pdto.pnum}')"></i> 
   ...
   <div class="text-left">
       <p class="h4 text-muted">${requestScope.pdto.pname} 제품 사용후기</p>
        ...
        <div class="col-lg-10">
          <form name="commentFrm">
              ...
              <input type="hidden" name="fk_userid" value="${sessionScope.loginUser.userid}" />
                  <input type="hidden" name="fk_pnum" value="${requestScope.pdto.pnum}" />
          </form>
       </div>
