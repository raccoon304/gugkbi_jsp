$(function(){
	...
	let start = 1;
	let b_loading = false;   // 추가: 중복 Ajax 호출 방지용
	const len = 8;           // 추가: 단위 고정
	const threshold = 10;    // 추가: 바닥 판정 여유값(px)
//	$('span#totalHITCount').hide();
//  	$('span#countHIT').hide();
  
  	// HIT상품 게시물을 "스크롤" 이벤트에 대한 초기값 호출하기 
	...
	$(window).scroll(function(){
			...
			// 기존 비교식 대신: 소수/오차 대응을 위한 ceil/floor + threshold 사용
			let scrollTop = Math.ceil($(window).scrollTop());
			let maxScroll = Math.floor($(document).height() - $(window).height());

			// 바닥 근처 도달시
			if (scrollTop >= maxScroll - threshold) {
				
				
				// 이미 Ajax 로딩중이면 또 호출하지 않음
				if (b_loading) return;
				b_loading = true;
				...
				// Ajax가 끝나기 전에 스크롤 이벤트가 또 들어오는 것 방지
				//    (displayHIT의 success/error에서 풀어주는 게 베스트지만, 최소수정으로 타이머로 풀어줌)
				setTimeout(function(){ b_loading = false; }, 300);
			}
			
			// 맨 위 리셋은 "start가 1보다 클 때만" 실행 (초기 로딩 시 scrollTop=0이라 리셋되는 것 방지)
			if (scrollTop <= 0 && start > 1) {

				$('div#displayHIT').empty();
				$('span#end').empty();
				$('span#countHIT').text("0");  // ✅ 추가: 카운트 초기화 (없으면 누적 꼬임)
	...
	$.ajax({
		...
		*/   
            ...
            */
			...
			// 스크롤을 계속하여 countHIT 값과 totalHITCount값이 일치하는 경우
...
function goTop(){
	scrollTop = Math.ceil($(window).scrollTop(0));
}